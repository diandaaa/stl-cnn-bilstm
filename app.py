...
with t3:
    ...
    sec("📊 Perbandingan: Model vs Mean Baseline")
    mean_baseline = np.full_like(y_te_true, MEAN_SST_TRAINVAL)
    mape_model = mape_fn(y_te_true, h_te)
    mape_mean  = mape_fn(y_te_true, mean_baseline)
    ...
    _dm = mape_mean - mape_model; _dm_mae = mae_mean - mae_model
    narasi(
        f"**CNN-BiLSTM+STL** vs **Mean Baseline** (prediksi konstan = {MEAN_SST_TRAINVAL:.3f}°C). "
        f"MAPE model **{mape_model:.4f}%** vs baseline **{mape_mean:.4f}%** "
        f"({'lebih baik' if _dm>0 else 'lebih buruk'} {abs(_dm):.4f} pp). "
        f"MAE model **{mae_model:.4f}°C** vs baseline **{mae_mean:.4f}°C** "
        f"({'lebih baik' if _dm_mae>0 else 'lebih buruk'} {abs(_dm_mae):.4f}°C)."
    )
