...
def fdGPH(x, bw=0.5):
    ...
    lam=2*np.pi*j/n; fv=np.fft.fft(x)
...
def generate_random_sst(n=4071, seed=42):
    ...
    trend   = 29.3 + 0.00008*t                              # naik sangat pelan
    seasonal= 0.35*np.sin(2*np.pi*t/365 - 0.3)             # seasonal kecil
    noise   = rng.normal(0,0.08,n)
    for i in range(1,n): noise[i]+=0.6*noise[i-1]
...
with t0:
    ...
    with col_norm:
        # normalisasi seluruh SST untuk preview
...
with t3:
    ...
    ax.plot(dates[lookback:n_train],     sp_tr,color=PAL["train"],lw=1.1,ls="--",label="Train")
...
with t4:
    ...
    for (label,(yt,yp)),col in zip(sets.items(),cols_m):
        mp=mape(yt,yp); ma=mae_fn(yt,yp)
        results[label]={"MAPE (%)":mp,"MAE":ma}
        col.markdown(f"**{label}**")
        mcard(col,f"MAPE",     f"{mp:.2f}%")
        mcard(col,f"MAE",      f"{ma:.4f}")
    ...
    elif te_mape<5:   kual="baik (1–5%)"
    elif te_mape<10:  kual="cukup (5–10%)"
    else:             kual="perlu perbaikan (> 10%)"
...
with t5:
    ...
    TM=st.session_state["TM"]; SM=st.session_state["SM"]
    sc_t=st.session_state["sc_t"]; sc_s=st.session_state["sc_s"]
    tl=st.session_state["tl_last"]; sl=st.session_state["sl_last"]

    # Extend window dengan hasil test
    tp_te=st.session_state["tp_te"]; sp_te=st.session_state["sp_te"]

    # window terakhir setelah seluruh data diproses
    ...
    sl_fut=s_all_s[-lookback:]

    FUTURE_STEPS=10
    last_date=dates[-1]
    freq_guess=pd.infer_freq(dates[:50])
    if freq_guess is None: freq_guess="D"
    ...
    # ── Tabel sebelum & sesudah denormalisasi ────────────────────────────────
    ...
    with col_a:
        ...
        st.dataframe(df_norm.style.format({
            "Trend (norm)":"{:.5f}","Seasonal (norm)":"{:.5f}"
        }),use_container_width=True)
