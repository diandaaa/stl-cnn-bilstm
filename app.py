...
# ═══════════════════════════════════════════════════════════════
# GENERATE DATA SINTETIS SST
# ═══════════════════════════════════════════════════════════════
def generate_random_sst(n=730, seed=42):
    """
    Generate data SST sintetis ringan dengan karakter realistis:
    - Tren linier lambat (+0.14°C per 10 tahun)
    - Dua harmonik musiman (periode 365 dan 182.5 hari)
    - Noise AR(1) berkorelasi (alpha=0.62) — bukan white noise
    - Di-clip ke rentang realistis [base-2, base+2]
    Default: 730 hari (~2 tahun), cukup untuk analisis STL + training ringan
    """
    rng  = np.random.default_rng(seed)
    t    = np.arange(n)
    base = 29.3
    trend_per_day = 0.14 / 3650
    trend    = base + trend_per_day * t
    seasonal = (0.30 * np.sin(2 * np.pi * t / 365 - 0.3)
                + 0.12 * np.sin(4 * np.pi * t / 365))
    noise = rng.normal(0, 0.09, n)
    for i in range(1, n):
        noise[i] += 0.62 * noise[i-1]
    sst = np.clip(trend + seasonal + noise, base - 2.0, base + 2.0)
    dates = pd.date_range("2022-01-01", periods=n, freq="D")
    return pd.DataFrame({
        "tgl": dates.strftime("%-m/%-d/%Y"),
        "sst": np.round(sst, 5)
    })
...
with st.sidebar:
    ...
    data_source = st.radio("Pilih sumber data:", ["🎲 Generate Otomatis", "📁 Upload CSV"])
    uploaded = None
    if data_source == "📁 Upload CSV":
        ...
        # Parameter generate — sederhana, langsung jalan
        date_col = "tgl"
        sst_col  = "sst"
        gen_n    = st.slider("Jumlah hari", 365, 1460, 730, 30,
                             help="730 hari (~2 tahun) sudah cukup untuk STL + training ringan")
        gen_seed = st.number_input("Seed", value=42, help="Ganti angka untuk variasi data berbeda")
...
if not run_btn:
    ...
    with st.expander("📋 Format CSV yang diterima (jika upload)"):
        s = generate_random_sst(10, 42)
        st.dataframe(s, use_container_width=True)
        st.download_button("⬇ Contoh CSV", s.to_csv(index=False).encode(), "contoh.csv", "text/csv")
    with st.expander("ℹ️ Mode Generate Otomatis"):
        st.markdown("""
Saat memilih **🎲 Generate Otomatis**, data SST sintetis dibuat langsung tanpa perlu upload file.

Karakter data yang dihasilkan:
- **Tren linier** lambat: +0.14°C per 10 tahun
- **Musiman tahunan**: dua harmonik (365 hari + 182.5 hari)
- **Noise AR(1)**: berkorelasi (α=0.62) — lebih realistis dari white noise
- **Rentang**: di-clip ke [base−2, base+2]°C
- **Default**: 730 hari (~2 tahun), cukup ringan untuk training lokal
        """)
    st.stop()
...
# ── LOAD / GENERATE DATA ──────────────────────────────────────
if data_source == "🎲 Generate Otomatis":
    df_raw = generate_random_sst(int(gen_n), int(gen_seed))
    st.success(f"✅ Data sintetis SST berhasil dibuat: **{len(df_raw):,} hari** "
               f"(seed={int(gen_seed)}, {pd.to_datetime(df_raw['tgl'].iloc[0]).strftime('%d %b %Y')} "
               f"– {pd.to_datetime(df_raw['tgl'].iloc[-1]).strftime('%d %b %Y')})")
else:
    if uploaded is None:
        st.error("⚠️ Upload CSV terlebih dahulu.")
        st.stop()
    df_raw = pd.read_csv(uploaded)

if date_col not in df_raw.columns or sst_col not in df_raw.columns:
    st.error(f"Kolom tidak ditemukan. Tersedia: {list(df_raw.columns)}")
    st.stop()
...
if n_test <= 0:
    st.error("Test set kosong. Kurangi val ratio atau tambah jumlah data.")
    st.stop()
...
with t0:
    ...
    with ca:
        ...
        rd = df[[sst_col]].reset_index().head(10).copy()
        rd.columns = ["Tanggal", "SST (°C)"]
    ...
    with cb:
        ...
        st.dataframe(nd.style.format({"SST (°C)": "{:.5f}", "Norm [0,1]": "{:.5f}"}),
...
with t1:
    ...
    for ax, (nm, val, col) in zip(axes, [
        ...
        ax.set_ylabel(nm, fontsize=9, color="#e2e8f0")
        ax.grid(True, lw=.4); tight_ylim(ax, [val])
    ...
    axes[1].text(d_gph + 0.04, 0, f"d = {d_gph:.4f}", va="center",
...
with t5:
    ...
    n_above_p90  = int(is_above_p90.sum())
    n_mhw        = int(is_mhw_fc.sum())

    sec(f"🔮 Forecast {STEPS} Hari ke Depan")
    fig, ax = plt.subplots(figsize=(14, 4))
    ax.plot(dates[-tail_days:], y_full[-tail_days:], color=PAL["actual"], lw=1.8,
            alpha=0.6, label=f"Aktual ({tail_days} hari terakhir)", zorder=2)
    ...
    tight_ylim(ax, [y_full[-tail_days:], hf])
    ...
    narasi(
        f"Proyeksi SPL untuk **{STEPS} hari** ke depan berada pada rentang "
    ...
    with st.expander("ℹ️ Informasi Tambahan – Anomali Termal & Potensi MHW"):
        st.markdown(
            f"Sebagai informasi tambahan, berikut posisi prediksi SPL terhadap "
            f"persentil ke-90 data historis (**{THRESHOLD_MHW_FULL:.3f}°C**) yang merupakan "
            f"ambang batas *Marine Heatwave* (MHW) menurut Hobday et al. (2016)."
        )
        ...
        axes3[1].annotate(f"{hf[idx_max]:.3f}°C",
        ...
        ref_box(
            "Ambang MHW = persentil ke-90 dari baseline historis, berlangsung ≥5 hari berturut-turut. "
            "Hobday, A.J., et al. (2016). <i>Progress in Oceanography</i>, 141, 227–238. "
